...
{  
  unsigned char  UART0RecData; //串口数据
  unsigned short adcData;      //队列数据
  unsigned char  waveData;     //波形数据
  
  static unsigned char s_iCnt4 = 0;            //计数器
  static unsigned char s_iPointCnt = 0;        //波形数据包的点计数器
  static unsigned char s_arrWaveData[5] = {0}; //初始化数组
  
  if(Get2msFlag())                             //判断2ms标志位状态
  { 
    if(ReadUART0(&UART0RecData, 1))            //读串口接收数据
    {       
      ProcHostCmd(UART0RecData);               //处理命令      
    }
