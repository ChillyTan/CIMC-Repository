...
typedef struct
{
  unsigned char packModuleId;    //模块包ID
  unsigned char packHead;        //数据头
  unsigned char packSecondId;    //二级ID
  unsigned char arrData[6];      //包数据
...
unsigned char    PackData(StructPackType* pPT);      //对数据进行打包，1-打包成功，0-打包失败
unsigned char    UnPackData(unsigned char data);                //对数据进行解包，1-解包成功，0-解包失败
