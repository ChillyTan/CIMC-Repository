...
*                                              内部函数声明
*********************************************************************************************************/
static  void ConfigTIMER2ForCapture(unsigned short arr, unsigned short psc);    //配置TIMER2

/*********************************************************************************************************
*                                              内部函数实现
*********************************************************************************************************/
/*********************************************************************************************************
* 函数名称：ConfigTIMER2ForCapture
* 函数功能：配置TIMER2
* 输入参数：arr-自动重装值，psc-预分频器值
* 输出参数：void
* 返 回 值：void
* 创建日期：2022年01月01日 
* 注    意：此处暂定使用定时器TIMER2的CH2（PB0）来做输入捕获，捕获PB0上高电平的脉宽
*********************************************************************************************************/
static  void ConfigTIMER2ForCapture(unsigned short arr, unsigned short psc)
{ 
  ...
  timer_deinit(TIMER2);                                     //TIMER2设置为默认值
  timer_struct_para_init(&timer_initpara);                  //TIMER2结构体设置为默认值
  ...
  timer_init(TIMER2, &timer_initpara);                      //初始化时钟
  ...
  timer_input_capture_config(TIMER2, TIMER_CH_2, &timer_icinitpara);//初始化通道

  timer_auto_reload_shadow_enable(TIMER2);                //使能自动重载影子寄存器

  timer_interrupt_flag_clear(TIMER2, TIMER_INT_FLAG_CH2); //清除CH2中断标志位
  timer_interrupt_flag_clear(TIMER2, TIMER_INT_FLAG_UP);  //清除更新中断标志位
  
  timer_interrupt_enable(TIMER2, TIMER_INT_CH2);          //使能定时器的CH2输入通道中断
  timer_interrupt_enable(TIMER2, TIMER_INT_UP);           //使能定时器的更新中断
  nvic_irq_enable(TIMER2_IRQn, 2, 0);                     //使能TIMER2中断，并设置优先级
...
{  
  ...
  {  
    ...
    if (timer_interrupt_flag_get(TIMER2, TIMER_INT_FLAG_CH2) != RESET) //发生捕获事件
    { 
      if(s_iCaptureSts & 0x40)  //bit6为1，即上次捕获到上升沿，那么这次捕获到下降沿
      {
        s_iCaptureSts |= 0x80;  //完成捕获，标记成功捕获到一次下降沿
        s_iCaptureVal = timer_channel_capture_value_register_read(TIMER2,TIMER_CH_2);  //s_iCaptureVa记录捕获比较寄存器的值
        
				//设置为上升沿捕获，为下次捕获做准备
        timer_channel_output_polarity_config(TIMER2, TIMER_CH_2, TIMER_IC_POLARITY_RISING);
      }
      else  //bit6为0，表示上次没捕获到上升沿，这是第一次捕获上升沿
      {
        ...
        timer_counter_value_config(TIMER2, 0);  //设置寄存器的值为0
                                                             
        s_iCaptureSts |= 0x40;    //bit6置为1，标记捕获到了上升沿
        
        //设置为下降沿捕获
        timer_channel_output_polarity_config(TIMER2, TIMER_CH_2, TIMER_IC_POLARITY_FALLING);
      }    
    } 
  }
 
  timer_interrupt_flag_clear(TIMER2, TIMER_INT_FLAG_CH2);  //清除更新CH2捕获中断标志位
  timer_interrupt_flag_clear(TIMER2, TIMER_INT_FLAG_UP);   //清除更新中断标志位
}
...
{
  //计数器达到装载值0xFFFF，会产生溢出；以240MHz/（239+1）=1MHz的频率计数
  ConfigTIMER2ForCapture(0xFFFF, 239);  
}
