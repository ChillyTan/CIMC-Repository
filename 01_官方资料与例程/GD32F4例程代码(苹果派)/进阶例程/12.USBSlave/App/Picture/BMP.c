...
*                                              内部函数声明
*********************************************************************************************************/
static u8   GetHeaderInFlash(const u8 *image);                         //获取文件头和信息头
static u8*  GetLineInFlash(const u8 *image, u32 line);                 //获取一行数据
static void BMPDisplayInFlash(StructBmpImage* image, u32 x, u32 y);    //获取Flash中的位图并显示
...
* 注    意：
*********************************************************************************************************/
static u8 GetHeaderInFlash(const u8 *image)
{ 
  u8 i;
...
* 注    意：
*********************************************************************************************************/
static u8* GetLineInFlash(const u8 *image, u32 line)
{
...
*********************************************************************************************************/
static void BMPDisplayInFlash(StructBmpImage* image, u32 x, u32 y)
{
  ...
  //读取BMP文件头和信息头
  GetHeaderInFlash((const u8*)image->addr);
  ...
  {
    //读取一整行数据
    line = GetLineInFlash((const u8*)image->addr, lineCnt);
...
{
  //图片储存在Flash中
  if(BMP_IN_FLASH == image->storageType)
  {
    BMPDisplayInFlash(image, x, y);
  }
}
