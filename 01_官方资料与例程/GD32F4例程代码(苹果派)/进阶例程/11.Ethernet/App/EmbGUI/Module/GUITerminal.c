...
{
  u16 x, y, i, width, height;

  //计算字符串的起始坐标和长宽
  x = s_structTherminal.x0 + x0;
  y = s_structTherminal.y0 + y0;
	width = s_structTherminal.width - 3 - x;
	height = s_structTherminal.textHeight;

  //清一行背景
  LCDFill(x, y, width, height, s_structTherminal.textBackColor);
  ...
  while((*(string + i) <= '~') && (*(string + i) >= ' '))
  {
    ...
    //到了一行的尽头
    if(x > (width + x - s_structTherminal.textHeight / 2))
    {
      break;
    }

    //打印输出到终端
    LCDShowChar(x, y, s_structTherminal.font, LCD_TEXT_TRANS, s_structTherminal.color, NULL,*(string + i));
    x += s_structTherminal.textHeight / 2;
    i++;
  }
}
...
void InitGUITerminal(void)
{
  ...
  s_structTherminal.width = 520;
  s_structTherminal.height = 400;
  s_structTherminal.color = LCD_COLOR_WHITE;
  s_structTherminal.font = LCD_FONT_24;
  s_structTherminal.textHeight = 24;
  DisplayBackground();
