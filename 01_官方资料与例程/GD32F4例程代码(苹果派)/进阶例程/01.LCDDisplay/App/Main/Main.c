...
static  void  Proc1msTask(void)
{
  ...
  if(Get1msFlag())
  {
    ...
    {
      s_iCnt = 0;
      wave = GetADC() * (s_structGraph.y1 - s_structGraph.y0) / 4095;
...
{ 
  ...
  {
    //printf("Proc1SecTask: wave time: %lld\r\n", s_iWaveTime);
    Clr1SecFlag();  //清除1s标志
  }
}
...
{  
  ...
  LCDShowString(30, 40, 210, 16, LCD_FONT_16, LCD_TEXT_TRANS, LCD_COLOR_BLUE, NULL, "Hello! GD32 ^_^");

  //显示背景图片
  DisPlayBackgroudJPEG();
