...
static u32 ReadData(void)
{
  ...
  //打开文件
  result = f_open(&s_fileBook, "0:/book/Holmes.txt", FA_OPEN_EXISTING | FA_READ);
  if (result !=  FR_OK)
  {
    printf("ReadBookByte: 打开指定文件失败\r\n");
    return 0;
  }
...
*********************************************************************************************************/
u32 SetPosition(u32 position)
{
  //读取位置超过文件大小
  if((position >= s_iBookSize) && (0 != s_iBookSize))
  {
    return 0;
  }

  //读入新一批数据
  s_iFilePos = position / BOOK_READ_BUF_SIZE;
  if(0 == ReadData())
  {
    return 0;
  }
  ...
  //更新读取字节计数
  s_iByteCnt = position % BOOK_READ_BUF_SIZE;
