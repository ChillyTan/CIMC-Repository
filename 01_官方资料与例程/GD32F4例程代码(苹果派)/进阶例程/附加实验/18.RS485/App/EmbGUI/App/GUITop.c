...
void InitGUI(StructGUIDev* dev)
{
  ...
  //串口号显示
  if(NULL != dev->serialPort)
  {
    LCDShowString(620, 60, 100, 16, LCD_FONT_16, LCD_TEXT_TRANS, LCD_COLOR_WHITE, NULL, (char*)dev->serialPort);
  }

  //显示波特率
  if(NULL != dev->baudRate)
  {
		LCDShowString(620, 80, 100, 16, LCD_FONT_16, LCD_TEXT_TRANS, LCD_COLOR_WHITE, NULL, (char*)dev->baudRate);
  }
...
void GUITask(void)
{
  ...
  if(GUI_BUTTON_NONE != button)
  {
    ...
    {
      if(0 == GetGUILineEditRusult(GUI_LINE_EDIT_SEND_DATA, &lineEdit))
      {
        s_pGUIDev->sendCallBack(lineEdit);
      }
    }
