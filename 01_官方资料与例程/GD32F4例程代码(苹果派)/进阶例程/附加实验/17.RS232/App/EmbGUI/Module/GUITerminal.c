...
*                                              内部变量
*********************************************************************************************************/
static StructGUITerminal 	s_structTerminal;                   //终端设备结构体
...
static void DisplayBackground(void)
{
  ...
  //初始化backgroundImage
  backgroundImage.image = (unsigned char*)s_arrJpegTerminalImage520x400x24;
  backgroundImage.size  = sizeof(s_arrJpegTerminalImage520x400x24) / sizeof(unsigned char);

  //解码并显示图片
  DisplayJPEGInFlash(&backgroundImage, s_structTerminal.x0, s_structTerminal.y0);
}
...
static void ShowString(char *string, u16 x0, u16 y0)
{
  ...
  //计算起始坐标和终点坐标
  x = s_structTerminal.x0 + x0;
  y = s_structTerminal.y0 + y0;
  x1 = s_structTerminal.x0 + s_structTerminal.width - 5;

  //清一行背景
  LCDFill(x, y, s_structTerminal.width - 5, 24, s_structTerminal.textBackColor);
  ...
  while((*(string + i) <= '~') && (*(string + i) >= ' '))
  {
    ...
    //到了一行的尽头
    if(x > (x1 - s_structTerminal.textSize / 2))
    {
      break;
    }
...
static void UpdateStringShow(void)
{
  ...
  else if(stringNum < LINE_NUM)
  {
    ...
    {
      ShowString(s_arrString[i], 3, 30 + 30 * i);
    }
  }
  ...
  else
  {
    ...
    {
      ShowString(s_arrString[head], 3, 30 + 30 * i);
...
void InitGUITerminal(void)
{
  ...
  //获取终端背景颜色
  x = s_structTerminal.x0 + s_structTerminal.width / 2;
  y = s_structTerminal.y0 + s_structTerminal.height / 2;
  s_structTerminal.textBackColor = LCDReadPoint(x, y);
}
