...
void InitGUI(StructGUIDev* dev)
{
  ...
  //显示服务器IP
  if(NULL != dev->serverIP)
  {
    GUIDrawTextLine(560, 56, (u32)dev->serverIP, GUI_FONT_ASCII_16, NULL, GUI_COLOR_WHITE, 1, 100);
  }

  //显示服务器端口
  if(NULL != dev->serverPort)
  {
    GUIDrawTextLine(560, 78, (u32)dev->serverPort, GUI_FONT_ASCII_16, NULL, GUI_COLOR_WHITE, 1, 100);
  }
...
void GUITask(void)
{
  ...
  if(GUI_BUTTON_NONE != button)
  {
    ...
    {
      if(0 == GetGUILineEditRusult(GUI_LINE_EDIT_CLIENT_IP, &ip))
      {
        if(0 == GetGUILineEditRusult(GUI_LINE_EDIT_SEND_DATA, &data))
        {
          s_pGUIDev->sendCallback(ip, data);
        }
      }
    }
  }
