...
static void DisplayBackground(void)
{
  ...
  //初始化backgroundImage
  backgroundImage.image = (unsigned char*)s_arrJpegTerminalImage440x700x24;
  backgroundImage.size  = sizeof(s_arrJpegTerminalImage440x700x24) / sizeof(unsigned char);
...
static void ShowString(char *string, u16 x0, u16 y0)
{
  ...
  x1 = s_structTherminal.x0 + s_structTherminal.width - 7;

  //清一行背景
  LCDFillPixel(x, y, x1, y + 24, s_structTherminal.textBackColor);
  ...
  while((*(string + i) <= '~') && (*(string + i) >= ' '))
  {
    ...
    //打印输出到终端
		LCDShowChar(x, y, LCD_FONT_24, LCD_TEXT_TRANS, s_structTherminal.textColor, NULL, *(string + i));
...
static void UpdateStringShow(void)
{
  ...
  else if(stringNum < LINE_NUM)
  {
    ...
    {
      ShowString(s_arrString[i], 7, 40 + 30 * i);
    }
  }
  ...
  else
  {
    ...
    {
      ShowString(s_arrString[head], 7, 40 + 30 * i);
...
void InitGUITerminal(void)
{
  ...
  s_structTherminal.x0 = 20;
  s_structTherminal.y0 = 80;
  s_structTherminal.width = 440;
  s_structTherminal.height = 700;
  s_structTherminal.textColor = LCD_COLOR_WHITE;
