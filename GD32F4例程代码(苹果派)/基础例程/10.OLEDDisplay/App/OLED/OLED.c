...
#define CLR_OLED_CS()   gpio_bit_reset(GPIOA,GPIO_PIN_4)   //CS，片选
#define SET_OLED_CS()   gpio_bit_set(GPIOA,GPIO_PIN_4)

#define CLR_OLED_SCK()  gpio_bit_reset(GPIOA,GPIO_PIN_5)   //SCK，时钟
#define SET_OLED_SCK()  gpio_bit_set(GPIOA,GPIO_PIN_5)
 
#define CLR_OLED_DC()   gpio_bit_reset(GPIOA,GPIO_PIN_6)   //DC，命令数据标志（0-命令/1-数据）
#define SET_OLED_DC()   gpio_bit_set(GPIOA,GPIO_PIN_6)

#define CLR_OLED_DIN()  gpio_bit_reset(GPIOA,GPIO_PIN_7)   //DIN，数据
#define SET_OLED_DIN()  gpio_bit_set(GPIOA,GPIO_PIN_7)
...
* 返 回 值：void 
* 创建日期：2022年01月01日
* 注    意：PC10（OLED_RES）、PA4（OLED_CS）、PA5（OLED_SCK）、PA6（OLED_DC）、PA7（OLED_DIN）
*********************************************************************************************************/
