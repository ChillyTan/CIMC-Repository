...
{
  unsigned char  packValid = 0;  //打包正确标志位，默认值为0

  packValid = PackData(pPackSent);  //打包数据
  
  if(0 < packValid)                 //如果打包正确
  {
    WriteUART0((unsigned char*)pPackSent, 10); //写数据到串口
  }
}
...
* 注    意：
*********************************************************************************************************/
void SendAckPack(unsigned char moduleId, unsigned char secondId, unsigned char ackMsg)
{
...
* 注    意：
*********************************************************************************************************/
void  SendWaveToHost(unsigned char* pWaveData)
{
  StructPackType  pt;               //包结构体变量
