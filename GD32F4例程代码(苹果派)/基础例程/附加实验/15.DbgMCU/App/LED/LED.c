...
* 注    意：
*********************************************************************************************************/
void PRESet(unsigned short pre)
{
  PRE = pre;
}
...
* 返 回 值：void
* 创建日期：2021年07月01日
* 注    意：LEDToggle在TIMER15_IRQHandler中调用，LED灯翻转周期为PRE*2 ms，
*********************************************************************************************************/
void LEDToggle(void)
{
  static unsigned short s_iPre;  //定义静态变量s_iCnt作为计数器
  
  s_iPre++;                      //计数器的计数值加1
  
  if(2*s_iPre >= PRE)              //计数器的计数值大于PRE
  { 
    s_iPre = 0;                  //重置计数器的计数值为0
