...
*                                              内部变量
*********************************************************************************************************/
static  unsigned char  s_arrRecData[MAX_REC_DATA_CNT]; //接收数据缓冲区
static  signed short   s_iDataCnt;                     //接收数据计数
static  unsigned char  s_iRecLineFeedFlag = 0;         //接收到换行符标志位，0-未接收到换行符，1-接收到换行符 
...
* 注    意:
*********************************************************************************************************/
unsigned char  CheckLineFeed(unsigned char data)
{  
  static unsigned char s_iRec0x0DFlag = 0;   //接收到0x0D标志，1-接收到了0x0D，0-未接收到0x0D (即回车键)

  if(0 == s_iRecLineFeedFlag)     //未接收完成，即未接收到“0x0D 0x0A”组合(回车换行)
  {
...
*********************************************************************************************************/
signed short GetRecData(unsigned char* pRecData)
{
  signed short i;  
  signed short retCnt;
