...
void InitLCD(void)
{
  ...
  gpio_output_options_set(GPIOH, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_8);
  ...
  gpio_output_options_set(GPIOH, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_10);

  //R5
  gpio_af_set(GPIOH, GPIO_AF_14, GPIO_PIN_11);
  gpio_mode_set(GPIOH, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO_PIN_11);
  gpio_output_options_set(GPIOH, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_11);
  ...
  gpio_output_options_set(GPIOH, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_15);
  ...
  gpio_output_options_set(GPIOI, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_1);

  //G7
  gpio_af_set(GPIOI, GPIO_AF_14, GPIO_PIN_2);
  gpio_mode_set(GPIOI, GPIO_MODE_AF, GPIO_PUPD_NONE, GPIO_PIN_2);
  gpio_output_options_set(GPIOI, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_2);
  ...
  gpio_output_options_set(GPIOI, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_4);
  ...
  //配置TLI时钟（时钟频率为1MHz * 240 / 4 / 4 = 15MHz，数据手册给的典型值是30MHz，但是15MHz刷新率下更稳定）
...
* 返 回 值：void
* 创建日期：2021年07月01日
* 注    意：扫描顺序是高位在前，从上到下，从左往右，一列最后不满一个字节的则补零
*********************************************************************************************************/
