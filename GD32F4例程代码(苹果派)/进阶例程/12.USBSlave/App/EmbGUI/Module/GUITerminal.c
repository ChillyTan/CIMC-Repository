...
*                                              内部变量
*********************************************************************************************************/
static StructGUITerminal g_structTerminal;                   //终端设备结构体
...
static void DisplayBackground(void)
{
  ...
  //初始化backgroundImage
  backgroundImage.image = (unsigned char*)s_arrJpegTerminalImage430x340x24;
  backgroundImage.size  = sizeof(s_arrJpegTerminalImage430x340x24) / sizeof(unsigned char);

  //解码并显示图片
  DisplayJPEGInFlash(&backgroundImage, g_structTerminal.x0, g_structTerminal.y0);
}
...
static void ShowString(char *string, u16 x0, u16 y0)
{
  ...
  //计算起始坐标和终点坐标
  x = g_structTerminal.x0 + x0;
  y = g_structTerminal.y0 + y0;
  x1 = g_structTerminal.x0 + g_structTerminal.width - 5;

  //清一行背景
  LCDFill(x, y, g_structTerminal.width - 5, 24, g_structTerminal.textBackColor);
  ...
  while((*(string + i) <= '~') && (*(string + i) >= ' '))
  {
    ...
    //到了一行的尽头
    if(x > (x1 - g_structTerminal.textSize / 2))
    {
      break;
    }

    //打印输出到终端
		LCDShowChar(x, y, LCD_FONT_24, LCD_TEXT_TRANS, LCD_COLOR_WHITE, NULL, *(string + i));
    x += g_structTerminal.textSize / 2;
    i++;
  }
}
...
static void UpdateStringShow(void)
{
  ...
  height = g_structTerminal.textHeight;
...
void InitGUITerminal(void)
{
  ...
  g_structTerminal.x0 = 25;
  g_structTerminal.y0 = 55;
  g_structTerminal.width = 430;
  g_structTerminal.height = 340;
  g_structTerminal.textColor = LCD_COLOR_WHITE;
  g_structTerminal.textSize = 24;
  g_structTerminal.textHeight = 30;
  DisplayBackground();

  //获取终端背景颜色
  x = g_structTerminal.x0 + g_structTerminal.width / 2;
  y = g_structTerminal.y0 + g_structTerminal.height / 2;
  g_structTerminal.textBackColor = LCDReadPoint(x, y);
}
