...
static  u8  s_arrSendBuf[UART0_BUF_SIZE];     //发送串口循环队列的缓冲区
static  u8  s_arrRecBuf[UART0_BUF_SIZE];      //接收串口循环队列的缓冲区
...
void USART0_IRQHandler(void)            
{
  ...
  {                                                         
    ...
    usart_data_receive(USART0);  //读取USART_DR 
  }                                                         
  ...
  {                       
    ...
    usart_data_transmit(USART0, uData);                  //将uData写入USART_DR
