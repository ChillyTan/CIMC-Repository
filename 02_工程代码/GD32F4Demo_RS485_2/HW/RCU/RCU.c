...
* 创建日期：2022年01月01日
* 注    意：配置的时钟如下：
*           （1）PLLCLK（PLL时钟）     = 240MHz
*           （2）SYSCLK（系统时钟）    = 240MHz
*           （3）HCLK（AHB总线时钟）   = 240MHz
...
static void ConfigRCU(void)
{
  ...
  if(HXTALStartUpStatus == SUCCESS)                   //外部晶振已经稳定
  {
    ...
    rcu_apb2_clock_config(RCU_APB2_CKAHB_DIV2);       //设置高速APB2时钟（PCLK2） = AHB/2
  
    rcu_apb1_clock_config(RCU_APB1_CKAHB_DIV4);       //设置低速APB1时钟（PCLK1） = AHB/4
  
    //设置锁相环PLL = HXTAL / 25 * 480 /2 = 240 MHz 
