...
static  unsigned char  s_arrSendBuf[UART1_BUF_SIZE];     //发送串口循环队列的缓冲区
static  unsigned char  s_arrRecBuf[UART1_BUF_SIZE];      //接收串口循环队列的缓冲区
...
* 返 回 值：void
* 创建日期：2022年01月01日
* 注    意：USART1-TX-AF7-PA2 USART1-RX-AF7-PA3
*********************************************************************************************************/
...
* 返 回 值：void 
* 创建日期：2022年01月01日
* 注    意：s_iUARTTxSts = UART_STATE_ON;这句话必须放在usart_interrupt_enable之前，否则会导致中断打开无法执行
*********************************************************************************************************/
...
void USART1_IRQHandler(void)            
{
  ...
  {                                                         
    ...
    usart_data_receive(USART1);                                         //读取USART_DR 
  }                                                         
  ...
  {                       
    ...
    usart_data_transmit(USART1, uData);                  //将uData写入USART_DR
